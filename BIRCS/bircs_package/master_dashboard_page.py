...
class MasterDashboardPage:
    def __init__(self, parent_frame, engine, root_window):
        ...
        self.sidebar_color = "#1D2153"
        self.text_muted = "#7A7A7A"

        # 🚀 STICKY STRUCTURE KAGAYA NG STAFF DASHBOARD!
        self.main_wrapper = ctk.CTkFrame(self.parent_frame, fg_color="transparent")
    ...
    def build_ui(self):
        ...
        ctk.CTkLabel(header_frame, text="Master Database & Analytics", font=("Arial", 28, "bold"),
    ...
    def build_table(self, container):
        ...
        ctk.CTkLabel(top_ctrls, text="Filter Records", font=("Arial", 14, "bold"), text_color=self.text_dark).pack(
            side="left")

        # 🚀 THE POGI FIX: STATUS FILTER PARA KAY KAPITAN
        ...
        ctk.CTkOptionMenu(top_ctrls, variable=self.filter_status_var, values=status_list, fg_color="white",
                          text_color=self.red, button_color="#F0F0F0", button_hover_color="#E0E0E0", width=140,
                          height=35, font=("Arial", 12, "bold"), command=lambda e: self.reset_and_refresh()).pack(
            side="left", padx=(15, 5))
        ...
        ctk.CTkOptionMenu(top_ctrls, variable=self.filter_category_var, values=cat_list, fg_color="white",
                          ...
                          height=35, command=lambda e: self.reset_and_refresh()).pack(side="left", padx=(5, 0))

        self.search_var = ctk.StringVar()
        self.search_entry = ctk.CTkEntry(top_ctrls, textvariable=self.search_var,
    ...
    def reset_and_refresh(self):
        ...
        # 🚀 PYTHON NINJA TRICK: SALA-IN NATIN BASE SA STATUS PARA DI NA KAILANGAN GALAWIN DB
        filtered = []
    ...
    def build_master_case_card(self, case):
        ...
        card = ctk.CTkFrame(self.table_rows_frame, fg_color="white", border_color=border_col, border_width=2,
        ...
        right_info.pack(side="right", padx=20, pady=12)

        ctk.CTkLabel(right_info, text=display_status, font=("Arial", 12, "bold"), text_color=border_col).pack(
            side="left", padx=(0, 20))
        ctk.CTkButton(right_info, text="View Details", width=100, fg_color="#F0F0F0", text_color=self.text_dark,
                      hover_color="#E0E0E0", command=click_cmd).pack(side="right")
    ...
    def show_incident_details(self, row_data):
        ...
        ctk.CTkLabel(header_frame, text=f"Case Number: {row_data.get('case_no')}", font=("Arial", 20, "bold"),
        ...
        fields = [
            ...
            ("Processed By:", row_data.get('processed_by')),
            ("Status:", row_data.get('status'))
        ]

        for i, (label, val) in enumerate(fields):
            ctk.CTkLabel(info_frame, text=label, font=("Arial", 12, "bold"), text_color="gray").grid(row=i, column=0,
                                                                                                     sticky="w",
                                                                                                     padx=15, pady=5)
            ctk.CTkLabel(info_frame, text=str(val), font=("Arial", 12), text_color=self.text_dark).grid(row=i, column=1,
                                                                                                        sticky="w",
                                                                                                        padx=10, pady=5)

        ctk.CTkLabel(scroll_frame, text="Narrative:", font=("Arial", 14, "bold"), text_color=self.text_dark).pack(
            anchor="w", pady=(20, 5))
        ctk.CTkLabel(scroll_frame, text=row_data.get('narrative'), font=("Arial", 12), text_color="black",
                     wraplength=700, justify="left").pack(anchor="w", padx=10)

        if row_data.get('reopen_status') == 'Requested':
            ctk.CTkLabel(scroll_frame, text="🚨 Re-open Request Pending", font=("Arial", 16, "bold"),
                         text_color=self.red).pack(pady=(30, 10))
            ctk.CTkLabel(scroll_frame, text=f"Reason: {row_data.get('narrative_2')}", font=("Arial", 12, "italic"),
    ...
    def build_bottom_analytics(self):
        ...
        ctk.CTkButton(top_ctrl, text="🖨️ Generate PDF Report", fg_color=self.primary, hover_color=self.dark_green,
        ...
        self.card_resolved = self.create_infograph_card(self.info_cards_container, 1, "Resolved", "0", "✅",
